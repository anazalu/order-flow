        ...
        stage('Build') {
            steps {
                script {
                    ...
                    bat "docker run -d -v '${WORKSPACE}:/workspace' -w /workspace -p 8000:8000 python:3.9 sh -c 'pip install --upgrade pip && pip install -r back/requirements.txt && cd back && python manage.py migrate && python manage.py runserver 0.0.0.0:8000 &'"

                    // Build and run the Frontend Docker container
                    bat "docker run -d -v '${WORKSPACE}:/workspace' -w /workspace -p 3000:3000 node:18 sh -c 'cd front && npm ci && npm start &'"
                }
            }
        }
